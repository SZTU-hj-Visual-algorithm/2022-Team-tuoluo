...
//空间点计算函数
Point3f GetSpaceCenter(Point3f a,Point3f b,Point3f c);                   //哪三点？                                                           //空间三点计算圆心
...
TuoluoData ShootTuoluo::getTuoluoData(Mat Src){
    ...
    if(finalArmor.status == FirstFind){
        //首次发现目标各项进行初始化
        firstSetTuoluo(angle,finalArmor);
        biaozhun = Point2f(finalArmor.tx,finalArmor.tz);
        return tuoluoData;
    }else if(finalArmor.status == Shoot){     //如果是处于射击状态
        double R;                           //运动半径
        Point2f center;                 //运动圆心
        if(ContinueSetTuoluo(Src,angle,finalArmor,R,center,tuoluoData.status,tuoluoData.runDirection,tuoluoData.spinSpeed)){
            //确认对方处于陀螺状态
#ifdef IMAGE_DRAWING
            //大致绘制出陀螺圆心坐标,存在一定的绘制误差   参数名要改
 ...
 * @param angle         所计算角度
 */
void ShootTuoluo::firstSetTuoluo(double angle,Final_Armor armor){
    //各项清零
    continueNumber = 0;
    circlePoints.clear();
    max_angle = 0;
...
bool ShootTuoluo::ContinueSetTuoluo(Mat Src, double &angle, Final_Armor BestArmor, double &R, Point2f &center, TuoluoStatus &status, TuoluoRunDirection & direction, float & spinSpeed){
...
//        imshow("左侧",Src);
    ...
    if(getPointTimes>LeastFitTimes){                            //当前已满足存储数量,开始更新
        ...
        circlePoints.push_back(Point2f(BestArmor.tx,BestArmor.tz));         //存入circlePoints,用于轨迹拟合,得到摆动范围  push_back是在数组的最后添加一个数据
    }
    //记录陀螺角度值,利用方差确定是否为陀螺
    ...
    if(circlePoints.size()>10&&continueNumber>=TUOLUO_CONTINUE_NUM_MIN&&VarianceIsBig){
...
#endif
        double proportion = sin(max_angle*PI/180)/(sin(max_angle*PI/180)+sin(-min_angle*PI/180));
        R = (2*radius*proportion)/sin(max_angle*PI/180);
        //计算圆心左侧距离
        ...
        if(status == SWITCH){
            ...
            if(direction == LEFT){          //当前左转
                if(newGet.angle - oldGed.angle>0){   //在左侧
